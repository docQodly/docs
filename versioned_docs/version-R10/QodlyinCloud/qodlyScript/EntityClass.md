...
The `.lock()` function <!-- REF #EntityClass.lock().Summary -->puts a pessimistic lock on the record referenced by the entity<!-- END REF -->. The [lock is set](../qodlyScript/guides/data.md#entity-locking) for a record and all the references of the entity in the current process.
