...
Allows navigating when we are inside a webform loader, or it can be simply used to know if the component is inside a webform loader
...
- `actions`: An object containing methods for manipulating datasource values, including [`getDatasource`](#actionsgetdatasource), [`setDatasourceValue`](#actionssetdatasourcevalue), [`setCurrentElementValue`](#actionssetcurrentelementvalue), [`fetchDatasourceValue`](#actionsfetchdatasourcevalue), and [`fetchCurrentElementValue`](#actionsfetchcurrentelementvalue).
:::`
...
This example is using useSources and useMemo to get the value of a datasource called `usernameDatasource`.
...
const Example: FC<ExampleProps> = ({ usernameDatasource }) => {
  const {
    actions: { getDatasource }
  } = useSources();
  const datasource = useMemo(() => getDatasource(usernameDatasource), [usernameDatasource]);
  const [value, setValue] = useState('');

  useEffect(() => {
   if (!datasource) return;
   const getValue = async () => {
     const val = await datasource.getValue();
     setValue(val);
   };
   datasource.addListener('changed', getValue);
   return () => {
     datasource.removeListener('changed', getValue);
   };
  }, [datasource]);
...
This example is using useSources and useDataloader to fetch simple list of people with `firstName` and `lastName`
...
This example is using useSources and useDataloader and fetches the initial entities for the first Index
...
const Example: FC = () => {
  ...
  const { entities, fetchIndex } = useDataLoader<Person>({
    source: ds,
  });
