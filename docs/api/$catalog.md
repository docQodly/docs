...
If you have defined [shared singletons](../language/basics/lang-classes.md#singleton-classes) containing at least one [exposed function](../guideCenter/programmingGuide/data-model.md#exposed-vs-non-exposed-functions), a `singletons` section is added to the returned json for both the `/$catalog` and `/$catalog/$all` syntaxes. It contains the collection of singleton classes as objects with their **name** and **methods** (i.e., exposed functions).
