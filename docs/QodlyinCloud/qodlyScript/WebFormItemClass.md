...
When you call the [`webForm`](./commands/webForm) command, the returned `4D.WebForm` proxy object holds as many `4D.WebFormItems` as there are components with [**server-side references**](../4DQodlyPro/pageLoaders/pageLoaderOverview.md#server-side) in your webform.
