...
- New [Page states](../studio/pageLoaders/states.md) feature. Page states provide developers with a versatile solution for dynamically applying a variety of changes to Qodly pages. This feature is particularly useful for modifying the user interface (UI), styling, and other properties on the fly.
...
- Image component - ability to [drag and drop an image from the shared folder](../studio/pageLoaders/components/image.md#drag-and-drop-from-shared-folder) into image source and the default image source.
- New [Clean](../studio/pageLoaders/events.md#qodly-sources) standard action: an entity selection can be cleaned from its deleted entities which avoid blank spaces on the UI for those entities
- [Contextual panel](../studio/pageLoaders/pageLoaderOverview.md#contextual-panel): function call actions can now be copied/pasted, and a called function name can be changed a posteriori (which avoids deleting and recreating it)
- [Singletons](../language/basics/lang-classes.md#singleton-classes) can be edited in Qodly studio, their functions can be called from a Qodly page and permissions can be set up for their functions.
...
- Scalar qodly source type can be changed a posteriori (which avoids deleting and recreating it)
