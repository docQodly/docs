...
- Ability to handle a function call with a [GET verb](../studio/pageLoaders/events/bindingActionToEvents#binding-onhttpget-functions). This allows, for instance, some download file features.
...
- New [`onHttpGet`](../orda/data-model.md#onhttpget-keyword) function keyword to define singleton or ORDA functions that can be called through REST requests with the [HTTP GET verb](../api/classFunctionsOverview.md#function-call-syntax).
